...
data_sub2=np.genfromtxt("spettro_substrato#2_antenna_d=16mm_pin=300_pout=300_Potenza=xx.DAT", skip_header=10, invalid_raise=False)*1.4


FSR=16.7948*1e9
x=np.linspace(-FSR/2,FSR/2,len(data_I1))

I_sott1=data_I1-(data_sub2)
I_sott1[I_sott1 < 0] = 0

#plt.plot(x,data_sub1, color="navy", label='Intensità substrato1')
#plt.show()
#plt.plot(x, data_sub2, color='navy', label='Intensità substrato2')
#plt.show()
plt.plot(x,data_sub2, color="navy",label="Intensità substrato")
plt.plot(x, data_I1, color="black",label="Intensità per una singola sfera")
plt.legend()
...
for l in lll:
    ...
    for n in range(2):
        ...
        if l==2 and n==0:
            plt.plot(freqzzz,spectrum*pow(10,34)/2, label=f"modo ({n+1},{l})",color="orange")
        else:
            plt.plot(freqzzz,spectrum*pow(10,34)/2, label=f"modo ({n+1},{l})")

#plt.plot(x, I_sott1, color="black", label="exp sottratto")
plt.plot(x, data_I1, color="black", label="exp")
plt.plot(x,data_sub2, color="navy",label="Intensità substrato")
plt.xlabel("Frequenza [Hz]")
